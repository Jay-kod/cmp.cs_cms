...
<div style="background: #1e293b; padding: 0.8rem 1.5rem; border-radius: 12px; margin-bottom: 1.5rem; display: flex; align-items: center; justify-content: space-between;">
    <span style="color: #94a3b8; font-size: 0.9rem;"><i class="fa-solid fa-circle-info" style="margin-right: 6px;"></i>Editing: <strong style="color: white;">Past HODs Page</strong></span>
    <a href="{{ route('past-hods') }}" target="_blank" style="background: var(--color-primary); color: white; padding: 0.4rem 1rem; border-radius: 8px; font-size: 0.85rem; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 0.4rem;"><i class="fa-solid fa-eye"></i> Preview</a>
</div>
...
<div class="pc-card">
    ...
    <div class="pc-card-body">
        <div class="form-group">
            ...
            <input type="text" name="past_hods_hero_title" value="{{ $s('past_hods_hero_title', 'Past Heads of Department') }}">
        </div>
        <div class="form-group">
            <label>Hero Subtitle</label>
            <textarea name="past_hods_hero_subtitle" rows="2">{{ $s('past_hods_hero_subtitle', 'Honoring the leaders who have driven our department forward.') }}</textarea>
        </div>
        <div class="form-group">
            <label>Hero Background Image</label>
            @if($s('hero_past-hods'))
            <div style="margin-bottom: 0.5rem;"><img src="{{ asset('storage/'.$s('hero_past-hods')) }}" style="height: 80px; border-radius: 8px; object-fit: cover;"></div>
            @endif
            <input type="file" name="hero_past-hods" accept="image/jpeg,image/png,image/webp">
        </div>
    </div>
</div>
