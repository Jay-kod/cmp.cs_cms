...
.nd-c-thread { margin-bottom: 0.2rem; }
.nd-c-item {
    display: flex;
    gap: 0.7rem;
    padding: 0.7rem 0;
...
.nd-c-meta { display: flex; align-items: center; gap: 0.35rem; flex-wrap: wrap; margin-bottom: 0.15rem; }
.nd-c-author { font-weight: 700; font-size: 0.82rem; color: #1e293b; }
.nd-c-you { font-size: 0.6rem; padding: 0.1rem 0.35rem; background: rgba(var(--color-primary-rgb,22,163,106),0.1); color: var(--color-primary); border-radius: 3px; font-weight: 700; }
.nd-c-time { font-size: 0.72rem; color: #94a3b8; }
.nd-c-text { font-size: 0.88rem; color: #334155; line-height: 1.55; margin: 0; word-wrap: break-word; }
.nd-c-actions { display: flex; align-items: center; gap: 0.9rem; margin-top: 0.3rem; }
.nd-c-reply-btn {
...
.nd-c-replies-wrap .nd-c-item { padding: 0.45rem 0; }
.nd-c-replies-wrap .nd-c-avatar {
    width: 28px; height: 28px;
    font-size: 0.62rem;
    background: linear-gradient(135deg, #f0fdf4, #dcfce7);
    color: var(--color-primary);
}
.nd-c-replies-wrap .nd-c-author { font-size: 0.78rem; }
.nd-c-replies-wrap .nd-c-text { font-size: 0.83rem; }
.nd-c-replies-wrap .nd-c-time { font-size: 0.68rem; }
.nd-c-replies-wrap .nd-c-reply-btn { font-size: 0.7rem; }
...
.nd-c-toggle-replies {
    ...
    margin: 0.15rem 0 0.1rem 2.7rem;
    transition: color 0.2s;
}
.nd-c-toggle-replies:hover { color: var(--color-primary); }
.nd-c-toggle-replies i { font-size: 0.6rem; transition: transform 0.25s; }
.nd-c-toggle-replies.expanded i { transform: rotate(90deg); }
.nd-c-toggle-line {
...
.nd-inline-reply {
    ...
    padding-left: 1rem;
    border-left: 2px solid rgba(var(--color-primary-rgb,22,163,106),0.2);
    display: none;
    gap: 0.4rem; align-items: flex-start;
}
.nd-inline-reply.visible { display: flex; }
...
.nd-inline-reply button:hover { opacity: 0.85; }

.nd-comments-empty { display: none; text-align: center; padding: 2rem 1rem; }
.nd-load-more { display: none; text-align: center; margin-top: 1rem; }
.nd-load-more button {
    padding: 0.45rem 1.3rem; border: 1.5px solid #e2e8f0;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSingleComment(c, isReply) {
        ...
        h += `<div class="nd-c-avatar">${getInitials(c.author_name)}</div>`;
        ...
        h += `<span class="nd-c-time">${c.created_at}</span></div>`;
        h += `<p class="nd-c-text">${c.body}</p>`;
        h += `<div class="nd-c-actions">`;
        h += `<button class="nd-c-reply-btn" onclick="showReplyForm(${c.id})"><i class="fa-solid fa-reply" style="margin-right:2px;font-size:0.65rem;"></i> Reply</button>`;
        h += `</div></div></div>`;
        return h;
    }
    ...
    function renderThread(c) {
        ...
        if (replyCount > 0) {
            ...
            h += `<span>View ${replyCount} ${replyCount === 1 ? 'reply' : 'replies'}</span>`;
            h += `<i class="fa-solid fa-chevron-right"></i>`;
            h += `</button>`;
        }
    ...
    window.toggleReplies = function(id) {
        const wrap = document.getElementById('replies-' + id);
    ...
    function loadComments(url) {
        fetch(url || commentsGetUrl, { credentials: 'same-origin' })
            .then(r => r.json()).then(data => {
                ...
                items.forEach(c => list.insertAdjacentHTML('beforeend', renderThread(c)));
    ...
    window.submitComment = function(e) {
        ...
        fetch(commentsPostUrl, {
            ...
            if (data.success) {
                ...
                document.getElementById('comments-empty').style.display = 'none';
                document.getElementById('comments-list').insertAdjacentHTML('afterbegin', renderThread(data.comment));
    ...
    window.showReplyForm = function(id) {
        document.querySelectorAll('.nd-inline-reply.visible').forEach(f => f.classList.remove('visible'));
        const f = document.getElementById('reply-form-' + id);
        if (f) { f.classList.add('visible'); f.querySelector('textarea').focus(); }
    };
        ...
        fetch(commentsPostUrl, {
            ...
            if (data.success) {
                ...
                if (toggle) {
                    ...
                    toggle.querySelector('span:nth-child(2)').textContent = `Hide ${count} ${count === 1 ? 'reply' : 'replies'}`;
                } else if (thread) {
                    const mainItem = thread.querySelector('.nd-c-item');
                    const toggleHtml = `<button class="nd-c-toggle-replies expanded" onclick="toggleReplies(${parentId})" id="toggle-${parentId}"><span class="nd-c-toggle-line"></span><span>Hide ${count} ${count === 1 ? 'reply' : 'replies'}</span><i class="fa-solid fa-chevron-right"></i></button>`;
                    mainItem.insertAdjacentHTML('afterend', toggleHtml);
                }
                const b = document.getElementById('comment-count-badge');
                b.textContent = parseInt(b.textContent||0) + 1;
            }
