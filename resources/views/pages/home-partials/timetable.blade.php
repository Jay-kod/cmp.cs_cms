    ...
    <div class="container" data-aos="fade-up">
        ...
        <div class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-0 border border-slate-100 overflow-hidden">
            @if((isset($timetables) && $timetables->count() > 0) || !empty($uploadedTimetable))
                ...
                <div class="tt-responsive-grid p-10 border-b border-slate-300 grid grid-cols-2 gap-12 bg-slate-200 items-start max-md:grid-cols-1 max-md:p-6">
                    <div>
                        ...
                        <div class="flex gap-4 flex-wrap">
                            <a href="{{ Storage::disk('public')->url($uploadedTimetable) }}" target="_blank" class="btn btn-primary inline-flex items-center justify-center gap-2.5 bg-[color:var(--color-primary)] text-white no-underline py-[0.8rem] px-6 rounded-lg font-bold text-[0.95rem] border-none transition-all duration-300 shadow-[0_4px_10px_rgba(22,163,74,0.2)] hover:shadow-[0_6px_15px_rgba(22,163,74,0.3)] hover:-translate-y-[2px]">
                                <i class="fa-solid fa-cloud-arrow-down"></i> Download File
                            </a>
                            <a href="{{ url('/resources') }}" class="btn btn-secondary bg-white text-slate-600 py-[0.8rem] px-6 rounded-lg no-underline font-bold text-[0.95rem] border border-slate-300 inline-flex items-center gap-2 transition-all duration-200 hover:bg-slate-100 hover:text-slate-800">
                        ...
                        <div class="bg-white rounded-xl p-4 border border-dashed border-slate-300 flex items-center justify-center min-h-[250px] bg-clip-padding shadow-[0_4px_12px_rgba(0,0,0,0.03)]">
                            @if(Str::endsWith($uploadedTimetable, ['.jpg', '.jpeg', '.png', '.webp', '.gif']))
                                <a href="{{ Storage::disk('public')->url($uploadedTimetable) }}" target="_blank">
                                    <img src="{{ Storage::disk('public')->url($uploadedTimetable) }}" alt="Timetable Preview" class="max-w-full max-h-[400px] rounded-lg shadow-[0_4px_15px_rgba(0,0,0,0.05)] object-contain cursor-pointer transition-transform duration-300 hover:scale-[1.02]">
                                </a>
                            @elseif(Str::endsWith($uploadedTimetable, ['.pdf']))
                                <iframe src="{{ Storage::disk('public')->url($uploadedTimetable) }}#toolbar=0" class="w-full h-[400px] border-none rounded-lg shadow-[0_4px_15px_rgba(0,0,0,0.05)]"></iframe>
                            @else
                ...
                <div class="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-0">
                    @foreach($timetables as $timetable)
                        <div class="group p-8 border-r border-b border-slate-100 flex flex-col transition-all duration-300 relative bg-white hover:bg-slate-50">
                            ...
                            <a href="{{ Storage::disk('public')->url($timetable->file_path) }}" target="_blank" class="tt-download inline-flex items-center justify-center gap-2.5 bg-slate-100 text-slate-700 no-underline py-[0.8rem] px-[1.2rem] rounded-lg font-bold text-[0.9rem] transition-all duration-300 group-hover:bg-[color:var(--color-primary)] group-hover:text-white">
                                <i class="fa-solid fa-cloud-arrow-down"></i> Download File
                            </a>
                        </div>
                    @endforeach
